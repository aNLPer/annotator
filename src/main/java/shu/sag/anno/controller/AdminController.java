...
public class AdminController {
    ...
    // 获取分派任务列表
    @RequestMapping("admin/usertask/list")
    @ResponseBody
    public Object usertaskList(@RequestHeader("token") String token, int currentIndex, int pageSize) {
        JSONObject res = new JSONObject();
        res.put("data", new JSONObject());
        String verifyRes = TokenUtil.verify(token);
        if (verifyRes.equals("-1")) {
            ...
            if (role.equals("1")) {
                // 获取分派任务列表
                List<UserTask> userTaskList = adminService.getAllUserTask(currentIndex, pageSize);
                int userTaskTotal = adminService.getAllUserTaskNum();
    ...
    // 获取分派任务页面参数（未完成）
    @RequestMapping("admin/usertask/asign")
    @ResponseBody
    public Object userTaskAsign(@RequestHeader("token") String token) {
        JSONObject res = new JSONObject();
        res.put("data", new JSONObject());
        String verifyRes = TokenUtil.verify(token);
        if (verifyRes.equals("-1")) {
    ...
    // 分派任务
    @RequestMapping("admin/usertask/asign.do")
    @ResponseBody
    ...
    // 取消分派任务
    @RequestMapping("admin/usertask/delete")
    @ResponseBody
    public Object deleteUserTask(@RequestHeader("token") String token, int id){
        ...
        if (verifyRes.equals("-1")) {
            ...
            if (role.equals("1")) {
                // 删除已分配的任务
                int status = adminService.deleteUserTaskByID(id);
                if (status == 1) {
                    res.put("code", "0");
                    res.put("message", "删除成功！");
                    return res;
